  ...
  return (
    <div>
        ...
        {({ values, errors, handleChange, handleSubmit, isSubmitting }) => (
          <form onSubmit={handleSubmit}>
            <div className='flex items-stretch'>
              <input id='searchTerm' type='text' value={values.searchTerm} onChange={handleChange} className='flex-grow-0 w-full p-4 text-lg border-b border-gray-200 focus:outline-none mr-4'
      ...
      <div>
        ...
        {React.Children.toArray(users.map((user: User) => (
          <UserRow onAddMember={onAddMember} disableAddButton={addMemberLoading} isMobile={isMobile} user={user} />
        )))}
      </div>
    </div>
  );
